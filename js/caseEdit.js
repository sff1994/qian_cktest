$(function(){
	$("#btnUApi").click(function(){
		$.ajax({
			url:qccr.config.global.rootUrl+"/cases/update",
	...
	$(".btn-send").click(function(){
		...
		$.ajax({
			url:qccr.config.global.rootUrl+"/api/run",
			headers: { "Authorization": $.cookie("sessionId") },
			data:$("[name='caseForm']").serialize(),
			type:'post',
			dataType:'json',
			success:function(ret){
				if(ret.status=="1"){
					$("[name='responseHeader']").html("<pre>"+ret.data.headers+"</pre>");
	...
	$(".restit-interrun").find("div").click(function(){
		...
		if(index==0){
			...
			//找到表中配置的断言规则,用于数据回显
			// var url = qccr.config.global.rootUrl+"/suite/findCaseTestRule";
